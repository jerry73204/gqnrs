...
use regex::Regex;
use image::{GenericImageView, FilterType, ImageFormat};
...
impl<'a> DeepMindDataSet<'a> {
    pub fn load_dir<P: AsRef<Path>>(
        ...
        ensure!(frame_channels.is_positive(), "frame_channels must be positive");
        ensure!(num_camera_params.is_positive(), "num_camera_params must be positive");
...
impl<'a> FileDataset<'a> {
    ...
    ) -> Fallible<FileDataset<'a>>
    {
        ...
        ensure!(image_paths.len() == timestamps.len(), "Image files and timestamp mismatch");

        // let trunc_size = image_paths.len() / sequence_size * sequence_size;

        let samples: Vec<_> = timestamps.into_iter()
        ...
        let train_iter = samples
            ...
            .par_map(move |mut example| {
                let (_, mut offsets_ref) = example
                    .remove_entry("offsets")
                    .unwrap();

                let offsets = offsets_ref.downcast_mut::<Vec<f32>>()
                    .unwrap();

                let query_offset = offsets.pop().unwrap();
                let query_offset_array = Array2::from_shape_vec(
                    (1, 1),
                    vec![query_offset],
                ).unwrap();
                let context_offsets_array = Array2::from_shape_vec(
                    (sequence_size - 1, 1),
                    offsets.to_vec(),
                ).unwrap();
                ...
                let frames_expanded: Vec<_> = frames
                    ...
                    .map(|array| array.insert_axis(Axis(0)))
                    .collect();
                let frame_views: Vec<_> = frames_expanded.iter()
                    .map(|array| array.view())
                    .collect();
                ...
                new_example.insert(
                    "context_frames".to_owned(),
                    Box::new(context_frames),
                );
                new_example.insert(
                    "target_frame".to_owned(),
                    Box::new(target_frame),
                );
                new_example.insert(
                    "context_params".to_owned(),
                    Box::new(context_offsets_array),
                );
                new_example.insert(
                    "query_params".to_owned(),
                    Box::new(query_offset_array),
                );
